...
export function VersionPanel({ workspaceId: _ws }: Props) {
  ...
  if (loading) return <div className="dashboard-loading">Versiyon geçmişi yükleniyor...</div>
  if (error) return <div className="dashboard-error"><p>{error}</p><button onClick={loadData}>Tekrar Dene</button></div>

  return (
    <div className="monitoring-panel">
      <div className="monitoring-header">
        <h3>🔖 Versiyon Takibi</h3>
        <p className="monitoring-desc">Model, engine ve konfigürasyon versiyon değişiklik geçmişi.</p>
      </div>
        ...
        <div>
          ...
          <div style={{ background: '#f8fafc', padding: '1.25rem', borderRadius: '10px' }}>
            <h4 style={{ fontSize: '1rem', fontWeight: 600, marginBottom: '1rem' }}>
              {selectedEntry.entity_name || selectedEntry.entity_id} — Versiyon Değişikliği
            </h4>
            <div style={{ display: 'grid', gridTemplateColumns: '140px 1fr', gap: '0.5rem', fontSize: '0.85rem' }}>
              <span style={{ color: '#64748b', fontWeight: 600 }}>Varlık Tipi:</span>
              <span>{selectedEntry.entity_type}</span>
              <span style={{ color: '#64748b', fontWeight: 600 }}>Eski Versiyon:</span>
              <span style={{ fontFamily: 'monospace', background: '#fef2f2', padding: '0.1rem 0.4rem', borderRadius: '3px', display: 'inline-block', width: 'fit-content' }}>{selectedEntry.old_version || '-'}</span>
              <span style={{ color: '#64748b', fontWeight: 600 }}>Yeni Versiyon:</span>
              <span style={{ fontFamily: 'monospace', background: '#f0fdf4', padding: '0.1rem 0.4rem', borderRadius: '3px', display: 'inline-block', width: 'fit-content' }}>{selectedEntry.new_version || '-'}</span>
              <span style={{ color: '#64748b', fontWeight: 600 }}>Değişiklik Notu:</span>
            ...
            {selectedEntry.old_version !== selectedEntry.new_version && (
              <div style={{ marginTop: '1rem', padding: '0.75rem', background: '#eef2ff', borderRadius: '8px', fontSize: '0.85rem', color: '#4338ca' }}>
                ✔ Versiyon değişikliği tespit edildi ({selectedEntry.old_version} → {selectedEntry.new_version})
              </div>
            )}
          </div>
        </div>
        ...
        <>
          {filtered.length === 0 ? (
            <div className="rec-empty">
              ...
              <h4>Henüz versiyon kaydı yok</h4>
              <p>Model ve konfigürasyon değişiklikleri burada takip edilecek.</p>
            </div>
          ) : (
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.85rem' }}>
              <thead>
                <tr style={{ borderBottom: '2px solid #e2e8f0', textAlign: 'left' }}>
                  <th style={{ padding: '0.5rem' }}>Varlık</th>
                  <th style={{ padding: '0.5rem' }}>Tip</th>
                  <th style={{ padding: '0.5rem' }}>Eski</th>
                  <th style={{ padding: '0.5rem' }}>Yeni</th>
              ...
              <tbody>
                {filtered.map((e) => (
                  <tr key={e.id} style={{ borderBottom: '1px solid #f1f5f9', cursor: 'pointer' }}
                    ...
                    <td style={{ padding: '0.5rem', color: '#64748b' }}>{e.entity_type}</td>
