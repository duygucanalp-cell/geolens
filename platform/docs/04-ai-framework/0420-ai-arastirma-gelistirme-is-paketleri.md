...
| Çıktı | Yazılımda Nasıl Kullanılır | Örnek | Ne İşe Yarar |
|-------|----------------------------|-------|-------------|
| Matematiksel Model | Ölçüm sonuçları hesaplanırken bu formül kullanılır. 7 bileşenin her biri (Varlık Payı, Konum, Kaynak Payı, Rakip Bağlamı, Appearance, Sentiment, Competitive Visibility — 0409 v1.3 kanonik seti) ayrı ayrı hesaplanır, ağırlıklarıyla çarpılır ve toplanır. Formül `CalculateScore()` fonksiyonunda kodlanır; varsayılan ağırlıklar AHP ile belirlendi (`ml/geolens/vi/ahp.py`, `ml/data/vi_ahp_report.md`). Her hesaplama `calculation_runs` tablosuna kaydedilir (deterministik replay için). | **Hesaplama:** `VI = 0.30×0.85 + 0.20×0.60 + 0.15×0.90 + 0.15×0.70 + 0.10×0.50 + 0.05×0.80 + 0.05×0.95` → `VI = 0.255 + 0.12 + 0.135 + 0.105 + 0.05 + 0.04 + 0.0475 = 75.25` → CI: ±4 (v2, yayılıma bağlı) → `{value: 75, ci_low: 71, ci_high: 79}` → Dashboard'da "75 ±4" olarak gösterilir. | Visibility Index'in şeffaf, bilimsel formülü |
| Ağırlıklandırma | Her sektör için farklı ağırlık profili tanımlanır. Varsayılan profil tüm markalar için geçerlidir. Müşteri sektörünü seçtiğinde ilgili profil kullanılır (`ml/geolens/vi/profiles.py`). Ağırlıklar AHP ile belirlendi, sensitivity analysis raporuyla hangi ağırlığın skoru ne kadar etkilediği gösterilir. | **Varsayılan (AHP, 0409 v1.3):** `{P: 0.30, K: 0.20, Ky: 0.15, Rb: 0.15, Ap: 0.10, S: 0.05, Cv: 0.05}` <br> **E-ticaret:** `{P: 0.20, K: 0.15, Ky: 0.10, Rb: 0.10, Ap: 0.25, S: 0.10, Cv: 0.10}` (appearance ağırlıklı) <br> **Sağlık:** `{P: 0.15, K: 0.10, Ky: 0.20, Rb: 0.05, Ap: 0.10, S: 0.30, Cv: 0.10}` (sentiment ağırlıklı) <br> **Sensitivity:** Citation ağırlığı %5 artarsa → VI puan değişimi `profiles.sensitivity()` ile hesaplanır → `ml/data/vi_profiles_report.md`. | Keyfi ağırlıklar yerine veriyle belirlenmiş optimal ağırlıklar |
...
| 1.2 | 11.08.2026 | **İP-06 §Çıktı hizalaması:** 7 bileşen seti 0409 v1.3 kanonik isimlerine (Varlık Payı, Konum, Kaynak Payı, Rakip Bağlamı, Appearance, Sentiment, Competitive Visibility) ve AHP ile belirlenmiş ağırlıklara (%30/20/15/15/10/5/5) güncellendi. Örnek VI hesaplaması ve sektör profilleri `ml/geolens/vi/` uygulamasıyla eşitlendi. |